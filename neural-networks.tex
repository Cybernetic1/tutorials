...
接下来要用到更多 \textbf{线性代数}，我会在另一篇 tutorial 解释。 
...
矩阵是 \textbf{线性方程组} 的紧凑写法。 一粒神经元 = $\sigmoid \circ \boxed{线性组合}$，一条线性组合是矩阵中的一行。

每一个矩阵都代表一个 \textbf{linear transformation}：
\begin{equation}
...
我们说这些顶点是\textbf{吸引子} (attractors)：
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.75]{attractors.png}}}
\end{equation}

注意： 在 $\sigmoid$ 变换时，图像是从周围\textbf{无限}的空间压缩到 $[-1,+1]^2$ 这方格里：
\begin{equation}
...
以下是\textbf{一层}神经网络的变换，分解成 $W$ 和 $\sigmoid$ 两部分：
\begin{equation}
...
\section*{Decision boundary 的形状}

现在再考虑，经过一层之后，如果在 target space 有一条 \textbf{直线}（或者叫 linear discriminant），那么这条直线在 original space 会是什么形状？  这个形状就是 \textbf{decision boundary}，亦即是说，我们在 target 空间切了「线性的一刀」，在原空间这刀割的形状是怎样？ 
...
\end{equation}
这家族的曲线（亦即是「刀的形状」）plot 出来是这样的：
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.5]{cut-1.png}}}
\end{equation}
%\includemedia[
%  activate=onclick,
%  width=0.5\textwidth
%]{\includegraphics[scale=1.0]{movie.png}}{movie.flv}
注意，如果 $\sigmoid$ 加入参数 $k$ 变成 $\displaystyle \frac{1}{1 + e^{-k x}}$，则当 $k \rightarrow \infty$ 时， $\sigmoid$ 变成 step function，而这些曲线也逐渐变成「角」的形状：
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.5]{cut-2.png}}}
\end{equation}
这表示，在 $k \rightarrow \infty$ 时，decision boundary 会分割 4 个角位。  （但其实当 $k \rightarrow \infty$ 时，在 target 空间只存在 4 个离散的顶点，所以那直线会变成 undefined）
...
TO-DO:  多層網絡的 decision boundaries（需要用到神經網絡的 inverse）。  

\section*{和 Bayesian network 比较}

以上是对 NN 的「几何解释」，但其实有个更简单直接的解释是： NN 实现的是 \textbf{命题逻辑} 的一种简单形式，例如以下的 Bayesian network 也可以看成是 NN 的一粒神经元，每个 node 代表一个命题： 
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.6]{flat-Bayes-net.png}}}
\end{equation}
但神经元的 $\sigmoid \sum$ 运算太简单，不足以计算概率的准确值。 举例来说，在 Bayesian network 中有 ``explaining away'' 的现象，例如以下的「遗嘱」如果存在，会令「自杀」的机率提升，但也令「谋杀」的机率下降：
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.6]{suicide-note.png}}}
\end{equation}
换言之，在 Bayesian network 中概率的计算是有传递性的，它需要用到复杂的 belief propagation 算法，但神经网络的简单性似乎难做到这效果。
...
\end{equation}
Decision tree 是根据 attribute-value pairs 来进行分类的。 为了较接近和神经网络比较，我假设这 decision tree 的每个 attribute 都是一个\textbf{线性分割}：
\begin{equation}
\sum a_i x_i \stackrel{?}{>} 0
\end{equation}
Decision tree 有个特点： 每次分割之后，分开的两边是\textbf{独立}处理的，彼此不再相关：
\begin{equation}
\vcenter{\hbox{\includegraphics[scale=0.6]{decision-tree-cuts.png}}}
\end{equation}
例如在最上层做了~ 男生／女生~ 的分割后，接下来女生的分割可能是「漂亮」、「身材好」这些属性，而男生那边则是「高大」、「有钱」等； 男生和女生的重要属性不同。 可以说决策树做的是 ``\textbf{shallow cut}''，和神经网络的 ``\textbf{deep cut}'' 不同。

观察多层 NN 的表达式：
\begin{equation}
\boxed{output} \; \vect{y} = \sigmoid \stackrel{1}{W} \; \sigmoid \stackrel{2}{W} \; ..... \; \sigmoid \stackrel{L}{W} \; \vect{x}
\end{equation}
可以看到，每一层的「分类」结果，\textbf{全部}送到下一层再作分类。 这和 decision tree 的情况很不同： 后者在建立 \textbf{分支} (branching) 之后，对每个分支的分类是独立的，即所谓 greedy algorithm。 这凸显出神经网络是一种很特殊的分类器，它自底层到高层、每层之间有「千丝万缕」的关系，所以能做到 representation learning 或 features learning 的效果。
